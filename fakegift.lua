--[[
SPAISPACEHUBZZZ fake gift v8 (Fixed Gamepass Flow, Real Blox Fruits UI Emulation)
--]]
...
-- Tiền khai báo hàm xử lý
local showProcessingModal

-- ============================================================
--  HỆ THỐNG GIAO DIỆN KHUNG ĐÚNG 100% BLOX FRUITS
-- ============================================================

-- Khung 1: Blox Fruits Confirm (Chỉ hiện GIFT và CANCEL)
local function showBloxFruitsConfirm(gamepassData)
    ...
    -- Nút GIFT (Màu vàng đất Blox Fruits)
    local giftBtn = Instance.new("TextButton")
    giftBtn.Size = UDim2.fromOffset(130, 42)
    giftBtn.Position = UDim2.new(0.3, -65, 0.72, 0)
    ...
    -- Nút CANCEL (Màu đỏ sẫm Blox Fruits)
    local cancelBtn = Instance.new("TextButton")
    cancelBtn.Size = UDim2.fromOffset(130, 42)
    cancelBtn.Position = UDim2.new(0.7, -65, 0.72, 0)
    ...
    giftBtn.MouseButton1Click:Connect(function()
        confirmGui:Destroy()
        -- Tiến hành mở khung danh sách người chơi gửi quà giống video
        showBloxFruitsGiftMenu(gamepassData)
    end)
end

-- Khung 2: Blox Fruits Send Gift Player List (Giống hệt video)
            ...
            if p ~= localPlayer then
                if filter == "" or p.Name:lower():find(filter:lower()) or p.DisplayName:lower():find(filter:lower()) then
                    ...
                    clickBtn.MouseButton1Click:Connect(function()
                        giftGui:Destroy()
                        -- Chuyển qua giao diện nạp tiền và thông báo troll giống Fruit
...
-- ============================================================
--  SENDING GIFT NOTIFICATION (Giữ nguyên form troll)
-- ============================================================
...
-- ============================================================
--  PURCHASE COMPLETED MODAL (Chuẩn Roblox Dark Theme)
-- ============================================================
...
-- ============================================================
--  BUY ITEM MODAL (Chuẩn Roblox Dark Theme)
-- ============================================================
...
-- ============================================================
--  CUTE SETTINGS UI GIAO DIỆN HỒNG DỄ THƯƠNG & ẨN UI (HIDE)
-- ============================================================
...
-- ============================================================
--  HỆ THỐNG PHÁT HIỆN CLICK VÀ HOOK CÁC NÚT TRONG GAME
-- ============================================================
local hookedBtns    = {}
...
local function hookGamepassShopButtons(btn)
    ...
    -- Quét xem nút bấm có tên trùng khớp cơ sở dữ liệu Gamepass không
    local targetGamepass = nil
    for _, f in ipairs(Fruits) do
        if f.isGamepass then
            local gpName = f.name:lower()
            -- So khớp tên nhãn hoặc tên vật thể trong UI gốc Blox Fruits
    ...
    -- Quét các Label con lân cận nếu nút gốc chỉ chứa hình ảnh
    if not targetGamepass and btn.Parent then
        local p = btn.Parent
        for _, f in ipairs(Fruits) do
            if f.isGamepass then
                local gpName = f.name:lower()
                if p.Name:lower():find(gpName:gsub("%s+", "")) then
                    targetGamepass = f
                    break
                end
    ...
    -- Nếu tìm thấy đúng nút của Gamepass gốc trong UI Shop
    if targetGamepass then
        hookedBtns[btn] = true
        
        -- Tạo một đè nút tàng hình để chặn click gốc mua thật của game
        ...
        overlay.MouseButton1Click:Connect(fireAction)
        btn.MouseButton1Click:Connect(fireAction)
    end
end

-- Vẫn giữ Core hook dự phòng cho Fruit hoặc hệ thống Roblox gốc
local function hookConfirmButton(btn)
    ...
    overlay.MouseButton1Click:Connect(function()
        -- Tiến trình tự động định vị vật phẩm Fruit đang thao tác
        local targetFruit = Fruits[1]
        showProcessingModal(fruitToProductInfo(targetFruit), "Unknown")
    end)
end

-- Quét toàn bộ UI hiện tại và liên tục lắng nghe UI mới
