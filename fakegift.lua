--[[
SPAISPACEHUBZZZ fake gift v12 (Fix lỗi không click được tên người chơi)
--]]
...
local function checkIsRobuxButton(btn)
    ...
    for _, child in ipairs(btn:GetChildren()) do
        -- Nút thanh toán thật bắt buộc phải chứa icon Robux
    ...
    -- Chỉ hook nếu nút đó chắc chắn có icon Robux (Bỏ qua nút tên người chơi vì không có icon Robux)
    if hasRobuxIcon then return true end
    return false
end
...
local function scanAndHookButtons(btn)
    ...
    -- Không chặn nút mở "Gift Menu" ban đầu
    if text == "gift" or text == "tặng" or name == "gift" or name == "giftbutton" then return end
    
    -- Thuật toán V12: CHỈ chặn nút có chứa Icon Robux (Bỏ qua hoàn toàn nút chọn người chơi)
    local isRobuxBtn = checkIsRobuxButton(btn)
