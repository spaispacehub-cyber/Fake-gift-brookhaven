--[[
SPAISPACEHUBZZZ fake gift v11 (Fix Bomb Glitch & 100% Native Gamepass Gift UI)
--]]
...
-- ============================================================
--  SMART SCANNER: Tự động tìm trái/gamepass trên màn hình
-- ============================================================
...
local function findTargetProduct(btn)
    local current = btn
    -- Quét lên trên 10 lớp parent để tìm tên item
    for i = 1, 10 do
    ...
    -- Dự phòng: Quét toàn bộ màn hình
                ...
                if child:IsA("TextLabel") then
                    ...
                    for _, plr in ipairs(Players:GetPlayers()) do
                        if plr ~= localPlayer then
                            if text:find(plr.Name) or text:find(plr.DisplayName) then
                                return plr.Name
                            end
                        end
                    end
...
cuteLabel(card2, "Interceptor Delay", 12, 14, CUTE_TEXT_MAIN, true)
...
-- ============================================================
--  HỆ THỐNG PHÁT HIỆN CLICK & NATIVE UI HOOKING
-- ============================================================
...
local function checkIsRobuxButton(btn)
    local name = btn.Name:lower()
    if name == "buy" or name == "buybutton" or name == "purchase" then return true end
    
    local hasRobuxIcon = false
    for _, child in ipairs(btn:GetChildren()) do
        if child:IsA("ImageLabel") and (child.Image:find("Robux") or child.Image == "rbxassetid://11560341145") then
            hasRobuxIcon = true
        end
    end
    
    -- Chỉ hook các nút nhỏ thực sự là Confirm Button, bỏ qua Grid Items
    if hasRobuxIcon and (name:find("btn") or name:find("button") or name:find("buy")) then return true end
    return false
end
...
local function scanAndHookButtons(btn)
    ...
    local isGiftModal = checkIsInsideGiftModal(btn)
    
    -- QUAN TRỌNG: KHÔNG CHẶN nút mở "Gift Menu". Cho phép mở thẳng UI gốc của game!
    if not isGiftModal and (text == "gift" or text == "tặng" or name == "gift" or name == "giftbutton") then
        return
    end
    
    local isRobux = checkIsRobuxButton(btn)
    local isGiftAction = isGiftModal and (text ~= "cancel" and text ~= "close" and name ~= "close")
    
    -- Chặn nút Buy bằng Robux VÀ chặn nút Click Người Chơi trong bảng Gift gốc
    if isRobux or isGiftAction then
        applyAbsoluteBlocker(btn, function()
            processIntercept(btn, isGiftAction)
        end)
    end
end

-- Quét toàn bộ màn hình liên tục để tóm nút mới
