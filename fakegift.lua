--[[
SPAISPACEHUBZZZ fake gift v7 (Dynamic Real-Time Asset Ripping & Settings Layout)
--]]
...
-- ============================================================
--  HOOK CORE & DYNAMIC MARKETPLACE ASSET RIPPING
-- ============================================================
local hookedBtns    = {}
local lastFruit     = nil
local lastRecipient = ""
        ...
        if self == MarketplaceService and
           (method == "PromptGamePassPurchase" or method == "PromptProductPurchase" or method == "PromptPurchase") then
            ...
            task.spawn(function()
                ...
                if fruit then
                    productData = fruitToProductInfo(fruit)
                elseif id then
                    -- Dynamic API Ripping for anything not in the list
                    local success, realInfo = pcall(function()
                        return MarketplaceService:GetProductInfo(id, infoType)
                    end)
                    
                    if success and realInfo then
                        local iconId = realInfo.IconImageAssetId or realInfo.AssetId or 0
                        local iconUrl = BLOX_LOGO
                        
                        if iconId > 0 then
                            iconUrl = "rbxthumb://type=Asset&id=" .. iconId .. "&w=150&h=150"
                        end

                        productData = {
                            Name         = realInfo.Name or "Unknown Item",
                            AssetId      = id,
                            PriceInRobux = realInfo.PriceInRobux or 0,
                            Icon         = iconUrl,
                            IsGamepass   = (infoType == Enum.InfoType.GamePass),
                        }
                    end
                end

                if productData then
                    showProcessingModal(productData, r)
                end
            end)
            
            -- We intercepted the call, completely block the real prompt from showing up
            return 
        end
        return old(self, ...)
    end))
