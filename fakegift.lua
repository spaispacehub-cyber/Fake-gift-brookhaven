--[[
SPAISPACEHUBZZZ fake gift v13 (Fix lỗi không hoạt động & chặn chính xác UI Blox Fruits)
--]]
...
local function fruitToProductInfo(fruit)
    if not fruit then fruit = Fruits[1] end
    return {
...
--  SMART SCANNER
-- ============================================================
local function findItemByName(text)
    ...
    for _, f in ipairs(sortedFruits) do
        local fname = f.name:lower():gsub("%p", " ")
...
local function findTargetProduct(btn)
    local current = btn
    for i = 1, 10 do
        if not current then break end
        if current.Parent then
            for _, child in ipairs(current.Parent:GetDescendants()) do
                if child:IsA("TextLabel") or child:IsA("TextButton") then
                    local found = findItemByName(child.Text)
    ...
    local screen = btn:FindFirstAncestorOfClass("ScreenGui")
    if screen then
...
local function findRecipient(btn)
    local current = btn
    for i = 1, 6 do
        if not current then break end
        if current.Parent then
            for _, child in ipairs(current.Parent:GetDescendants()) do
                if child:IsA("TextLabel") then
                    ...
                    for _, plr in ipairs(Players:GetPlayers()) do
                        if plr ~= localPlayer and (text:find(plr.Name) or text:find(plr.DisplayName)) then
                            return plr.Name
                        end
                    end
                end
...
local function showPurchaseDone(productInfo, recipient)
    ...
    if isGift then
        task.delay(0.5, function() showGiftNotification(productInfo.Name, recipient) end)
    end
end
...
for _, n in ipairs({"Spacehub_setting", "Spacehub_BuyModal", "Spacehub_Processing"}) do
...
userNameLbl.Size = UDim2.new(1, -90, 0, 20)
userNameLbl.Position = UDim2.fromOffset(80, 30)
userNameLbl.Font = Enum.Font.GothamBold
userNameLbl.TextSize = 15
userNameLbl.TextColor3 = CUTE_TEXT_MAIN
...
-- ============================================================
--  NEW HOOKING SYSTEM (V13)
-- ============================================================
local function applyAbsoluteBlocker(btn, actionFunc)
    if btn:GetAttribute("SpaceHubHooked") or btn:FindFirstChild("_FakeOverlayBlocker") then return end
    btn:SetAttribute("SpaceHubHooked", true)
    ...
    overlay.MouseButton1Click:Connect(function()
        task.spawn(function() actionFunc(btn) end)
    end)
end

local function checkIsInsideGiftModal(btn)
    local p = btn.Parent
    for _ = 1, 10 do
        if not p then break end
        local name = p.Name:lower()
        if name:find("gift") then return true end
        for _, child in ipairs(p:GetChildren()) do
            if child:IsA("TextLabel") then
                local t = child.Text:lower()
                if t:find("gifting") or t:find("send a gift") or t:find("tặng quà") or t:find("món quà") then
                    return true
                end
            end
        end
...
local function checkShouldIntercept(btn)
    ...
    -- Lấy chữ trên nút
    if btn:IsA("TextButton") then btnText = btn.Text end
    if btnText == "" then
        local txtChild = btn:FindFirstChildWhichIsA("TextLabel", true)
        if txtChild then btnText = txtChild.Text end
    end
    
    -- Bỏ hết các tag <font> hay ký tự thừa đi để check cho chuẩn
    btnText = btnText:gsub("<[^>]+>", ""):match("^%s*(.-)%s*$"):lower()
    
    -- RULE 1: TUYỆT ĐỐI BỎ QUA nút Gift, Cancel và chọn Server
    if btnText == "gift" or btnText == "cancel" or name:find("gift") or name:find("cancel") or btnText == "server" or btnText == "global" then
        return false
    end
    
    -- RULE 2: Bắt chính xác nút BUY màu xanh (do nút này không có icon Robux)
    if btnText == "buy" or name == "buy" or name == "buybutton" then 
        return true 
    end
    
    -- RULE 3: Quét SÂU bằng GetDescendants để tìm Icon Robux (Bắt nút xác nhận Giá Tiền ở bước cuối)
    local hasRobuxIcon = false
    for _, child in ipairs(btn:GetDescendants()) do
        if child:IsA("ImageLabel") or child:IsA("ImageButton") then
            local img = child.Image:lower()
            if img:find("robux") or img:find("11560341145") then
                hasRobuxIcon = true
                break
            end
        end
...
local function processIntercept(btn, isGiftAction)
    local product = findTargetProduct(btn)
    local recipient = "Unknown"
    
    if isGiftAction then
        recipient = findRecipient(btn)
    else
        recipient = localPlayer.Name
    end
    
    showProcessingModal(fruitToProductInfo(product), recipient)
end

local function scanAndHookButtons(btn)
    if not btn:IsA("GuiButton") then return end
    if btn:IsDescendantOf(settingsGui) then return end
    
    if checkShouldIntercept(btn) then
        applyAbsoluteBlocker(btn, function()
            local isGift = checkIsInsideGiftModal(btn)
            processIntercept(btn, isGift)
        end)
    end
end

for _, v in ipairs(playerGui:GetDescendants()) do
    task.defer(function() scanAndHookButtons(v) end)
end

playerGui.DescendantAdded:Connect(function(d)
    task.delay(0.1, function() scanAndHookButtons(d) end)
end)
