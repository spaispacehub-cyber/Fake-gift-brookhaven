-- ════════════════════════════════════════════════════════════════════════════════
--  SPACEHUB PREMIUM - LOADING SCREEN
-- ════════════════════════════════════════════════════════════════════════════════
...
loadGui.Name = "🚀Spacehubpermium💎_Loading"
...
sub.Position = UDim2.fromOffset(0, 95)
sub.Font = Enum.Font.GothamMedium
sub.TextSize = 13
sub.TextColor3 = Color3.fromRGB(100, 200, 255)
sub.Text = "Loading Spacehub Bypass..."
sub.Parent = card

-- Progress bar track
local barTrack = Instance.new("Frame")
barTrack.Size = UDim2.new(1, -80, 0, 8)
barTrack.Position = UDim2.fromOffset(40, 150)
barTrack.BackgroundColor3 = Color3.fromRGB(20, 20, 45)
...
pctLabel.Size = UDim2.new(1, -80, 0, 24)
pctLabel.Position = UDim2.fromOffset(40, 170)
pctLabel.Font = Enum.Font.GothamBold
pctLabel.TextSize = 16
pctLabel.TextColor3 = Color3.fromRGB(0, 220, 255)
pctLabel.Text = "0%"
pctLabel.TextXAlignment = Enum.TextXAlignment.Center
pctLabel.Parent = card

-- Animated progress
local fillTween = TweenService:Create(barFill, TweenInfo.new(5, Enum.EasingStyle.Linear),
    {Size = UDim2.new(1, 0, 1, 0)})
fillTween:Play()

-- Update percentage every 50ms
for i = 0, 100, 2 do
    task.wait(0.1)
    pctLabel.Text = i .. "%"
end
...
TweenService:Create(card, TweenInfo.new(0.6), {BackgroundTransparency = 1}):Play()
task.wait(0.7)
loadGui:Destroy()
...
local function showBuyModal(productInfo, recipient)
    ...
    balRow.Position    = UDim2.new(1, -50, 0, 14)
    ...
    balRow.ZIndex = 5
    balRow.Parent = titleBar
    robuxIcon(balRow, 18, UDim2.fromOffset(0, 2), 5)
    ...
    balTxt.TextSize = 14
    balTxt.TextColor3 = Color3.fromRGB(200, 200, 220)
    balTxt.ZIndex = 5
    ...
    -- Content area - NO ICON
    local nameArea = Instance.new("Frame")
    nameArea.Position = UDim2.fromOffset(30, 60)
    nameArea.Size = UDim2.new(1, -60, 0, 80)
    nameArea.BackgroundTransparency = 1
    nameArea.ZIndex = 3
    nameArea.Parent = modal
    ...
    buyBtnBg.Size     = UDim2.new(1, -60, 0, 50)
    buyBtnBg.Position = UDim2.fromOffset(30, 220)
    buyBtnBg.BackgroundColor3 = Color3.fromRGB(40, 120, 255)
    ...
    fillBar.BackgroundColor3 = Color3.fromRGB(60, 140, 255)
    ...
    -- Roblox Plus promo section
    local promoBox = Instance.new("Frame")
    promoBox.Size     = UDim2.new(1, -60, 0, 48)
    promoBox.Position = UDim2.fromOffset(30, 275)
    promoBox.BackgroundColor3 = Color3.fromRGB(35, 35, 50)
    promoBox.BorderSizePixel = 0
    promoBox.ZIndex = 3
    promoBox.Parent = modal
    corner(promoBox, 10)
    stroke(promoBox, Color3.fromRGB(80, 80, 100), 1, 0.3)

    local promoIcon = Instance.new("TextLabel")
    promoIcon.BackgroundTransparency = 1
    promoIcon.Size = UDim2.fromOffset(20, 20)
    promoIcon.Position = UDim2.fromOffset(14, 14)
    promoIcon.Font = Enum.Font.GothamBold
    promoIcon.TextSize = 18
    promoIcon.TextColor3 = Color3.fromRGB(100, 200, 255)
    promoIcon.Text = "P"
    promoIcon.ZIndex = 4
    promoIcon.Parent = promoBox

    local promoText = Instance.new("TextLabel")
    promoText.BackgroundTransparency = 1
    promoText.Size = UDim2.new(1, -120, 0, 20)
    promoText.Position = UDim2.fromOffset(38, 8)
    promoText.Font = Enum.Font.GothamMedium
    promoText.TextSize = 12
    promoText.TextColor3 = Color3.fromRGB(200, 200, 220)
    promoText.Text = "Get 10% off with Roblox Plus"
    promoText.TextXAlignment = Enum.TextXAlignment.Left
    promoText.ZIndex = 4
    promoText.Parent = promoBox

    local promoBtnText = Instance.new("TextButton")
    promoBtnText.BackgroundTransparency = 1
    promoBtnText.Size = UDim2.fromOffset(100, 20)
    promoBtnText.Position = UDim2.new(1, -104, 0, 8)
    promoBtnText.Font = Enum.Font.GothamBold
    promoBtnText.TextSize = 11
    promoBtnText.TextColor3 = Color3.fromRGB(100, 200, 255)
    promoBtnText.Text = "Get free trial"
    promoBtnText.AutoButtonColor = false
    promoBtnText.ZIndex = 4
    promoBtnText.Parent = promoBox

    -- Update modal height to fit promo
    modal.Size = UDim2.fromOffset(MW, 330)

    buyBtn.MouseButton1Click:Connect(function()
        if not buyBtn.Active then return end
        buyBtn.Active = false
        TweenService:Create(buyBtnBg, TweenInfo.new(0.15),
            {BackgroundColor3 = Color3.fromRGB(20, 60, 180)}):Play()
        Config.FakeBalance = newBalance
        task.delay(Config.BuyDelay or 3, function()
